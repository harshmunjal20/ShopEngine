...
const FeaturedProducts = ({featuredProducts}) => {
   // now we should have state to keep track of its index
   ...
   useEffect(() => {
      ...
      return () => window.removeEventListener('resize', handleResize); // cleanup => when component goes away (unmounts) cleanup means that listener exists only when needed
   },[]);
   ...
   const prevSlide = () => {
      setCurrentIndex((prevIndex) => prevIndex - itemsPerPage); // also we will create diabled and enabled below :)
   }

   const isStartDisabled = currentIndex === 0;
   const isEndDisabled = (currentIndex + itemsPerPage) >= (featuredProducts?.length || 0);
      ...
      <div className='container mx-auto px-4'>
         ...
         <div className='relative'>
            <div className='overflow-hidden'>
               ...
               > 
                  {/*(100 / itemsPerPage) is the width of one item */}
