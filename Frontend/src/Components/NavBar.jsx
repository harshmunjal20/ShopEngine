import {ShoppingCart , UserPlus, LogIn, LogOut, Lock} from 'lucide-react'; // lucide-react
import {Link} from 'react-router-dom';  // what link does is that unlike <a> which loads the whole page but Link updates only the ui (spa behaviour) => single opage application
// lucide react for icons 
/*
Simple meaning:

A website that loads only once and after that changes content without reloading the whole page.
            ...
            <div className='container mx-auto px-4 py-3'>
                <div className = 'flex flex-wrap justify-between items-center'>
                    ...
                    <nav className ='flex flex-wrap items-center gap-4 '>
                        ...
                        {user ? (
                            <button className = 'bg-gray-700 hover:bg-gray-600 text-white py-2 px-4 rounded-md flex items-center transition duration-300 ease-in-out' onClick={logout}> {/* on click we will run the logout function from the useUserStore */}
                                <LogOut size = {18}/>
                                <span className = 'hidden sm:inline ml-2'>Log Out</span>
                            </button>
                        ) : (
                            <>
                                <Link to={'/signup'} className = 'bg-emerald-600 hover:bg-emerald-500 text-white py-2 px-4 rounded-md flex items-center transition duration-300 ease-in-out'>
