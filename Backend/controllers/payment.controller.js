...
export const createCheckoutSession = async (req, res) => {
    try {
        ...
        if (couponCode) {
            coupon = await Coupon.findOne({code : couponCode, user_id : req.user._id, isActive : true}); // finding in database
            if (coupon) {
        ...
        } // 200 dollars in cents => 20000 we want that we should give discount 10% to the user in next purchase if he purchase more than 200 dollars in total
        res.status(200).json({id : session.id, totalAmount : totalAmount / 100}) // to get in dollars => totalAmount / 100 => we have sent sessionId here because it will be used in the frontend to display
    }
    catch (error) {
...
async function createNewCoupon(userId) {
    const newCoupon = new Coupon({
        code:"GIFT" + Math.random().toString(36).substring(2, 8).toUpperCase(),  // toString(36) means it will be in base 36, substring(2,8) removes the 0. part
